...
public class RadarChart implements Chart {
	...
	public String getChartFile(String filename, GUIResources resources,
		...
		String template = Util.fileToString(resources.resources()
				+ "template_html/radar_template.html");
		...
		for (int g = 0; g < groups.size(); g++) {
			...
			dataBuilder.append(",fillColor: 'rgba(" + color.getRed() + ","
					+ color.getGreen() + "," + color.getBlue() + ",0.25)'");
		...
		StringBuffer legendBuilder = new StringBuffer();
		...
		int newHeight = boundary.height
				+ (boundary.height / 8 - boundary.height / 43);
		template = template.replace("{CHART.JS}", chartJS);
		template = template.replace("{WIDTH}", boundary.width + "px");
		template = template.replace("{HEIGHT}", newHeight + "px");
		template = template.replace("{LEGEND-MARGIN}", "-"
				+ (boundary.height / 10) + "px");
		template = template.replace("{SHOWSCALE}", detailed + "");
		template = template.replace("{SHOWLABEL}", detailed + "");
		template = template.replace("{SHOWLINE}", detailed + "");
